...
def run(context):
    ...
    try: 
        ...
        for i in range (1,21):
            ...
            tempX = data[0][0]-.0001
            tempY = data[0][1]+(tempX-data[0][0])*(data[1][1]-data[0][1])/(data[1][0]-data[0][0])
            points.add(adsk.core.Point3D.create(tempX*100,tempY*100))

            for j in range (len(data)):
                # Define the points the spline with fit through.
                points.add(adsk.core.Point3D.create(data[j][0]*100,data[j][1]*100,))

            tempY = data[0][1]+(tempX-data[0][0])*(data[len(data)-2][1]-data[0][1])/(data[len(data)-2][0]-data[0][0])
            points.add(adsk.core.Point3D.create(tempX*100,tempY*100))            
